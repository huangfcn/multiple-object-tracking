...
namespace sp
{
	...
	arma_inline arma::fvec cos_win_f(const arma::uword N, const arma::vec & a) 
	{
		arma::fvec h(N);
		for (arma::uword i = 0; i < N; i++)
		{
            double ha;
            ha = a[0] - 
				a[1] * std::cos(1.0 * PI_2 * i / (N - 1)) +
                a[2] * std::cos(2.0 * PI_2 * i / (N - 1)) -
                a[3] * std::cos(3.0 * PI_2 * i / (N - 1)) +
                a[4] * std::cos(4.0 * PI_2 * i / (N - 1));
            h[i] = (float)ha;
		  }
        return h;		
	}
    ...
    /// @param N Nr of taps
    ////////////////////////////////////////////////////////////////////////////////////////////
    arma_inline arma::fvec hann_f(const arma::uword N) 
	{
      arma::vec a = arma::zeros<arma::vec>(5);
      a[0] = 0.5;
      a[1] = 0.5;
      return cos_win_f(N, a);
    }
    ////////////////////////////////////////////////////////////////////////////////////////////
